...
import { FileText, Image as ImageIcon, ArrowRight, TrendingUp, AlertCircle, X, LoaderCircle, History } from 'lucide-react';
...
const UploadPage: React.FC<UploadPageProps> = ({ dataCount, onFileUpload, onImageUpload, onProcessTextData, onGoToResult, onSetRusherPersona, isLoading, onGoToHistory }) => {
    ...
    const hasInvalidImage = useMemo(() => invalidImageWarning, [invalidImageWarning]);
    ...
    const removeImage = (urlToRemove: string) => {
        ...
        // If the user removes the image that caused the warning, reset the warning
    ...
    const totalDataCount = dataCount + localDataCount;
    const canProceed = (totalDataCount > 0 || uploadedImages.length > 0) && !isLoading;
      ...
      <div className="min-h-screen bg-gradient-to-b from-blue-50 to-white flex flex-col">
        <header className="bg-white border-b border-gray-200">
            <div className="max-w-2xl mx-auto px-5 py-4 flex items-center justify-between">
                ...
                <button onClick={onGoToHistory} className="flex items-center gap-2 text-sm font-semibold text-gray-600 hover:text-blue-600 transition-colors p-2 rounded-lg hover:bg-gray-100">
                    <History size={20} />
                    <span>분석 기록</span>
                </button>
            </div>
                ...
                {dataCount > 0 && (
                    <div className="bg-blue-100 rounded-xl p-3 mb-4 text-center border border-blue-200">
                        <p className="font-semibold" style={{ color: '#1E40AF' }}>
                            총 <span className="text-2xl font-bold" style={{color: PRIMARY_BLUE}}>{dataCount}</span>개의 소비 데이터가 입력되었습니다.
                        </p>
                    </div>
                )}
                <button
